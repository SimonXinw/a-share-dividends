...
- 表格列：**股票代码 / 股名 / 行业 / 当前股价 / 数据日期 / 当前总市值 / 去年年末总市值 / 预估 PE / 去年 / 去年每股分红 / 去年股息率 / 去年净利润 / 今年预估净利润 / 今年预估每股分红 / 今年预估股息率 / 备注**
...
今年预估 PE（无量纲倍数，保留两位小数）
    = 当前总市值（元）/ 今年预估全年净利润（元）
```

> **市值口径**：`current_market_cap` / `last_year_end_market_cap` 均为**总市值**（非流通市值）。当前总市值优先取东方财富 `stock_individual_info_em` 的「总市值」；若缺失则回退为 `股价 × 总股本`；去年末总市值 = 去年末收盘价 × 当前总股本（股本变动较少时近似合理）。**升级后请重新点「同步股价（含市值）」刷新库内旧数据。**

> **预估 PE 不落库**：与「去年股息率」「今年预估股息率」一样，由 `current_market_cap` 与 `this_year_estimated_profit` 在 `calculator.py` 运行时推导，无需新增 SQL 列或同步逻辑。
...
- 当前股价 / 去年末收盘价：`ak.stock_zh_a_daily(symbol="sh600519")`（新浪日线）
- 总股本 / 总市值：`ak.stock_individual_info_em(symbol="600519")`（东方财富个股资料）
...
- **跳过只看同步日期是否最新（天级别）**：`price_sync_date` / `fundamental_sync_date` 与今天同一天就跳过，不是最新日期就拉取。
